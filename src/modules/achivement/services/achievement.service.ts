...
export class AchievementService {
  ...
  async getNextAchievement(userId: number): Promise<NextAchievementOutput> {
    ...
    if (!nextAchievement) {
      return null; // No next achievement available
    }
  ...
  async getUserAchievements(userId: number): Promise<AchievementOutput[]> {
    ...
    const achievementOutputs = userAchievements.map(achievement =>
      plainToInstanceCustom(AchievementOutput, achievement.achievement),
    );
