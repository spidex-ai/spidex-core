...
export class UserQuestConsumerController {
  ...
  constructor(private readonly userQuestConsumerService: UserQuestConsumerService) {}
  ...
  @EventPattern(USER_QUEST_EVENT_PATTERN.SOCIAL_QUEST_VERIFY)
  async handleSocialQuestVerifyEvent(@Payload() data: ISocialQuestVerifyEvent, @Ctx() context: RmqContext) {
    const channel = context.getChannelRef();
    const originalMsg = context.getMessage();
    try {
      await this.userQuestConsumerService.handleSocialQuestVerifyEvent(context, data);
    } catch (error) {
      this.logger.error(`Error handling social quest verify event: ${error}`, error);

      const deadLetterMessage: IDeadLetterMessage<ISocialQuestVerifyEvent> = {
        key: data.userId.toString(),
        message: data,
        deadLetterReason: (error as Error).message,
        stack: (error as Error).stack,
      ...
      // Send to dead letter queue
      await this.userQuestConsumerService.handleSocialQuestVerifyEventDeadLetter(context, deadLetterMessage);
    }

    await channel.ack(originalMsg);
  }

  @EventPattern(USER_QUEST_EVENT_PATTERN.DEAD_LETTER.SOCIAL_QUEST_VERIFY)
  async handleSocialQuestVerifyEventDeadLetter(
    @Payload() message: IDeadLetterMessage<ISocialQuestVerifyEvent>,
    @Ctx() context: RmqContext,
  ) {
    const channel = context.getChannelRef();
    const originalMsg = context.getMessage();
    try {
      await this.userQuestConsumerService.handleSocialQuestVerifyEventDeadLetterRetry(context, message);
    } catch (error) {
      this.logger.error(`Error handling social quest verify event dead letter: ${error}`, error);
      message.deadLetterReason = (error as Error).message;
      message.stack = (error as Error).stack;
      message.retryCount++;
      await this.userQuestConsumerService.handleSocialQuestVerifyEventDeadLetter(context, message);
    }
    await channel.ack(originalMsg);
  }
}
