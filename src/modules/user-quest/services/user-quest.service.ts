...
export class UserQuestService {
  ...
  async handleSocialQuestVerifyEvent(data: ISocialQuestVerifyEvent): Promise<void> {
    const userQuest = await this.userQuestRepository.findOne({
      where: { id: data.userQuestId, deletedAt: IsNull() },
      relations: ['quest'],
    });
    if (!userQuest) {
      this.logger.error(`User quest not found during verification: ${data.userQuestId}`);
      return;
    }
    await this.completeQuest(data.userId, userQuest.quest, { userQuestId: userQuest.id });
  }
