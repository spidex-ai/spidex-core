...
export class TokenService {
  ...
  async getTokenDetails(tokenId: string): Promise<TokenDetailsResponse> {
    ...
    let data;
    try {
      data = await this.dexHunterService.getTokenDetail(tokenId);
    } catch (error) {
      console.warn('DexHunter getTokenDetail failed:', error);
      data = {
        is_verified: false,
        creation_date: new Date().toISOString(),
        policy: '',
  ...
  async searchTokens(request: TokenSearchRequest): Promise<SearchTokenResponse[]> {
    const { query, limit, page, verified } = request;
    ...
    const tokensWithDetails = tokens
      .map(token => {
        const tokenDetail: SearchTokenResponse = {
          ...
          token_ascii: token.project_name || mapTokenWithDetails[token.token_id]?.name,
          token_decimals: mapTokenWithDetails[token.token_id]?.decimals || 0,
          token_policy: mapTokenWithDetails[token.token_id]?.policy || '',
        };
        return tokenDetail;
      })
